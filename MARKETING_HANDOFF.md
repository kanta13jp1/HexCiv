...
2026-08-25判定: Stage 4Mは **本番計測中の販売候補**。P1本番行確認とテスト行削除、
P2実ゲーム画像3枚の本番公開が完了し、H1/H2の計測を開始した。H5はitch.io非公開下書きの
作成・レビュー待ち、H4/H7は外部初回投稿前なので未開始。`sales_candidate.json` と
`scripts/audit_sales_candidate.ps1` を正本候補監査に使う。自己購入1件は自然販売KPIから除外する。

2026-08-25の本番集計は、検証用 `probe_claude/product_view` 1行を削除後、
`direct/product_view` 3人、`purchase_click` 0人。画像公開前は2人・0押下、公開後は1人・0押下で、
H1/H2とも結論に必要な母数へ達していない。
...
`MARKETING_EXPERIMENT_H4_H7.md` に保存した。P1本番行確認とP2画像公開は完了したため開始ゲートは
通過済み。初回投稿の本文・画像・URLをユーザーが送信直前に確認した後で計測を開始し、開始後は
価格・商品ページ見出し・購入ボタンを同時変更しない。
...
### 販売ページ本番公開・計測確認（2026-08-22〜25）

- 商品ページPR [#4702](https://github.com/kanta13jp1/my_web_app/pull/4702) をマージし、
  [deploy run 32579800151](https://github.com/kanta13jp1/my_web_app/actions/runs/32579800151) が
  2026-08-22 23:51 JSTに成功した。
- `https://my-web-app-b67f4.web.app/shop/hexciv` でターン30／80／150の実ゲーム画像、¥500税込、
  買い切り、Windows版ダウンロード導線を表示し、ブラウザエラー0件を確認した。これをP2完了とする。
- 2026-08-25に特権SQLの集計だけを読み取り、`probe_claude/product_view` 1行を対象限定の短い
  transactionで削除した。削除後は `direct/product_view=3` のみ。これをP1完了とする。
- P2公開前は `product_view=2`／`purchase_click=0`、公開後は `product_view=1`／
  `purchase_click=0`。H1/H2は計測中だが、母数不足のため支持・反証は保留する。

## Claude Codeの次アクション

1. itch.ioアカウントと通貨・支払設定を読み取り確認し、非公開Draftへ630×500カバー、確認済み
   画像3～5枚、製品ZIP、無料Demo ZIPを登録する。フォーム送信前に対象・価格・公開範囲を提示する。
2. 日本語のみであることを、購入ボタンより前に表示する。
3. H4/H7の事前登録12投稿から1本目を、本文・画像・UTMを固定して送信前確認後に公開する。
4. `itch_io`、`direct`、`x` の4段ファネルを週次でObsidianへ記録する。
5. 外部公開・価格変更・有料広告は、対象と実験IDを記録してから実行する。

### H5 itch.ioアクセス状況（2026-08-25）

P1/P2は完了した。itch.ioはログイン状態・出品フォーム・決済通貨を読み取り確認中で、非公開Draftは
まだ送信していない。CAPTCHA、本人確認、資格情報または公開操作が必要な場合はユーザーへ引き渡す。
H5の開始日は公開ページが閲覧可能になった日とし、それ以前は訪問0ではなく未計測とする。

2026-08-25の確認では `kanta13jp1` でログイン済み、新規ProjectフォームとDraft保存は利用可能。
ただし支払設定が未完了で、価格を0より大きくしても購入者がダウンロードできない警告が出ている。
Publisher利用規約の承諾と決済接続はユーザー本人の確認・操作を必要とする。出品用カバーは
`MarketingAssets/itchio/hexciv_itchio_cover_630x500_v1.png` に作成済み。実ゲーム画面由来、630×500、
SHA256 `6577f134dad257cce3a9544498949e10e91032761a441d6f035bd6387ec0f32f`。

### H4/H7 Xアクセス状況（2026-08-25）

Chromeでは `@kanta13jp1` へログイン済みだが、Xがアカウントを一時的なread-only modeへ制限しており、
投稿・Repost・Likeを実行できない。事前登録1本目の本文・画像・UTMは確定済みだが、投稿不能なので
H4/H7は「開始待ち」のまま。制限解除までは別チャネルの数字をX実験へ混ぜず、訪問0とも数えない。
