...
第7群では季節洪水、橋梁工兵、海上封鎖、護送船団、港湾経営を中心に16系統を追加し、累計130系統とした。第8段階時点では沿岸の交戦中戦力を艦隊・沿岸砲・私掠活動の集約表現として扱った。
...
## シミュレーションゲーム設計参照索引（第8群）

第8群では個別艦船、艦隊運用、港湾建造、海戦、制海、船団護衛を中心に16系統を追加し、
累計146系統とした。数値表・艦影・UIは複製せず、「海上ユニットが地図上の補給と封鎖を
直接変える」という設計接続だけをHexCiv向けに実装する。

| 系統 | 代表的な参照作品 | 抽出する設計要素 | HexCivでの状態 |
|---|---|---|---|
| 歴史 | Uncharted Waters | 航海、港市、交易、船舶更新 | 港・航海技術・艦船済み、明示航路は候補 |
| 歴史 | P.T.O. II | 艦隊、港湾、生産、海上補給 | **艦船生産・海戦・港湾補給を第9実装** |
| 歴史 | Expeditions: Conquistador | 遠征、船舶、補給、乗員 | 補給済み、輸送・乗員は候補 |
| 歴史 | Nantucket | 船員、航海、船舶改装、危険 | 艦船済み、乗員・改装は候補 |
| 軍事 | Rule the Waves 3 | 艦隊設計、造船予算、封鎖、技術競争 | **国庫・港・封鎖・基礎艦種を実装済み** |
| 軍事 | Victory at Sea Pacific | 艦隊移動、港、占領、補給 | **海上移動・艦隊戦・敵港封鎖を第9実装** |
| 軍事 | Naval Action | 帆走、艦種、港、海戦 | 帆走・艦船済み、風向・艦装は候補 |
| 軍事 | Cold Waters | 探知、航路、交戦、戦域目標 | 視界・海戦済み、海上探知は候補 |
| 軍事 | Harpoon | センサー、射程、交戦規則、編隊 | 視界・射程基盤済み、センサー・編隊は候補 |
| 軍事 | Fleet Command | 艦隊指揮、航空連携、索敵、任務 | 艦隊AI基盤済み、任務・航空連携は候補 |
| 軍事 | Sea Power | 海上交通、封鎖、船団、航空脅威 | **封鎖圧力・局地護衛を第9実装** |
| 軍事 | Carrier Command 2 | 艦隊拠点、補給、島嶼、指揮 | 海上補給済み、移動拠点は候補 |
| 経営 | Port Royale 3 | 港市、船団、交易路、護衛 | 港・市場・護衛済み、明示船団は候補 |
| 経営 | Winds of Trade | 帆船、港市、価格、航路 | 艦船・価格済み、風と航路は候補 |
| 経営 | Rise of Venice | 港市経済、船団、交易、政治 | 市場・港・政治済み、商人組織は候補 |
| 経営 | High Sea Saga | 船体、乗員、探検、海戦 | 艦船・海戦済み、乗員・探検任務は候補 |
...
| 2 | 兵站 | 都市からの補給到達、地形コスト、敵遮断、孤立、補給切れ、技術・穀物庫 | **陸上網・港湾拠点・海上補給・艦船封鎖・局地護衛・護送船団を実装済み**（道路タイル・明示船団は次拡張） |
...
## 第9実装: 歴史船舶台帳・艦船・海戦・局地護衛

- **歴史船舶**: 河川舟、航海カヌー、交易帆船、軍船を6地域×6件、計36件収録。名称、時代、造船伝統、用途、概要を安定IDで保持する。
- **技術と建造**: 帆走で港・ガレー船、航海術で三段櫂船・護送船団庁を解禁。艦船は水域隣接かつ港を持つ都市だけで生産し、隣接水域へ進水する。
- **領域移動**: 共通の `CanUnitEnter` 契約により陸軍は陸地、艦船は水域だけを移動する。経路探索、実移動、AIが同じ判定を使う。
- **海戦**: 現段階は艦船対艦船の近接海戦。艦船による都市直接占領と陸軍による対艦攻撃を禁止し、領域境界を明確にする。
- **封鎖と護衛**: 敵艦は封鎖圧力2、敵沿岸部隊・敵港は1。近くの味方艦は護衛力1として相殺し、護送船団庁は遮断に必要な実効圧力を2へ上げる。
- **AI**: 港数に応じて艦船を建造。交戦中は敵艦迎撃または敵港沖封鎖、平時は自国港沖警備を行う。
- **表示**: 五角形船体、文明色甲板、白帆、二条の航跡を共有メッシュとして実行時生成する。
- **セーブ**: 既存のユニット定義ID経路を使うためversion 16を維持。旧セーブの読込を変更しない。

## 画像・動画・音楽・音声生成技術の分類台帳

「生成」は手続き生成、シミュレーション、制作支援、機械学習による合成を含みます。製品名の網羅ではなく、技術系譜を追跡します。第9版は8系統を増補し、累計89技術系譜です。
...
| 画像 | パラメトリック船体合成 | hull profile、polygon fan、faction tint | **五角形船体と輪郭を第9実装** |
| 画像 | 手続き帆装合成 | mast frame、sail triangle、rig preset | **白帆の共有メッシュを第9実装** |
| 画像 | 航跡形状合成 | wake wedge、alpha trail、velocity alignment | **二条の淡い航跡を第9実装** |
| 動画 | 艦船経路運動 | water-domain path、position tween、formation target | **既存トゥイーンへ海上経路を第9接続**、編隊は候補 |
| 動画 | 風・帆応答 | wind vector、sail deformation、heading constraint | 風向システムと帆アニメーション候補 |
| 音楽 | 海上状態適応スコア | port activity、blockade tension、fleet layer | 港湾繁栄・封鎖緊張を自作BGMへ反映する候補 |
| 音声 | 船体・帆走物理合成 | creak resonator、filtered noise、rope impulse | 外部録音なしの帆・櫂・船体SE候補 |
| 音声 | 艦隊信号ソニフィケーション | bell grammar、horn interval、event priority | 進水・接敵・封鎖・護衛成功の自作通知SE候補 |
