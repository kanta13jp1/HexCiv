...
- 水利の共同補修を即時完了から **4期間の共同管理** へ拡張した。ウルクが葦1と水利労働10%以上を負担すると、請求側水路の状態と堆積を改善し、期限付き利用権を得る。満了時は利用権を返し、信頼・評判・外交履歴へ完了を記録する。
- 分水・共同管理の履行中に明示的な「破約」を追加した。破約、一方的拒否、分水不履行では、請求側AIが最大10%の労働を民兵へ移し、両勢力間の輸送危険度を10ポイント上げる。食料危機は優先しつつ、報復理由をAI台帳へ保持する。
- 拒否・破約・不履行後は、大麦1と葦1を請求側の実在備蓄へ移して再交渉できる。報復を緩和し、同じ観測原因を保ったまま分水・穀物補償・共同管理・拒否を再選択する。地域UIへ状態、報復、破約、再交渉ボタンを追加した。
- 史実キャンペーン進捗をversion 7へ更新。version 6には報復なし・再交渉0回を補完し、version 1からの連続移行も保持する。CoreはMonoBehaviour非依存、決定論を維持する。

**販売促進**

- `MARKETING_EXPERIMENT_H10_UPDATES.md` に6週間の事前登録を作成。公開更新＋変更履歴＋X告知の3週と基準3週を `更新・基準・基準・更新・更新・基準` に固定し、2週間ごとに1対1で比較する。
- 主指標を自己購入除外後の週次手取り売上とし、支持を「更新週1.5倍以上・3ペア中2ペア以上優位・総購入4件以上」、反証を「各条件100閲覧以上で差なしまたは劣位」、それ以外を保留とした。価格、主要画像、購入導線、投稿時刻は期間中固定する。
- H10の開始ゲートはP1本番行確認、P2画像公開、itch.io本人確認、H4/H7終了、製品/Demo検証完了。開始前の数字は0件でなく未計測とし、Obsidian正本を `準備中` へ更新した。

**検証（Unity 6000.3.20f1）**

- `UrukRegionalSimulationSmokeTest.Run`: `URUK REGIONAL SIMULATION SMOKE OK`。共同管理開始・利用権・4期間完了、破約、拒否／不履行報復、民兵配分、輸送危険度、現物再交渉、v6→v7／v1連鎖移行、物資・水量・労働保存、3 seed決定性を確認。平均 `0.11 ms/turn`。
- `UrukCampaignVerticalSliceSmokeTest.Run`: `URUK CAMPAIGN VERTICAL SLICE SMOKE OK`。
- `SmokeTest.Run`: `SMOKE OK`。`Logs/g4_regression.log` と `[Smoke]` / `SMOKE` 19行が完全一致。
- `DemoModeSmokeTest.Run`: `DEMO SMOKE OK`。30ターン制限と製品版セーブ互換を確認。
- 製品版 `BUILD OK: 98,060,533 bytes`、Demo `DEMO BUILD OK: 98,060,533 bytes`。各12秒起動し、Unity 6000.3.20f1・重大例外0件。
- 製品ZIP: 37,584,995 bytes、SHA256 `9e1d5c59a9391ace97b715f6ddc56f2a9b136315509d51f9b08a379e0e2cbcd7`。
- Demo ZIP: 37,587,690 bytes、SHA256 `09c1daf2ed9b68c1e87835fe3a32f814d947a219e0f58c42e66684fbad760192`。両manifestと実ZIPのサイズ・SHA256一致、README／LICENSE同梱、除外対象なしを確認。

**次の担当**

- Claude: ユーザー承認後に商品画像PR #4408の生成リリースノートだけを修正し、CI再実行・マージ・P2開始日を返す。itch.ioはユーザーがCloudflare本人確認とログインを完了した後、非公開ページへ最新2ZIPと画像3枚を登録する。
- Codex: 複数水利要求と第三者仲裁へ進む。P1/P2/H4/H7/itch.ioの開始ゲートを越えるまではH10を開始せず、外部投稿・価格変更・有料広告を行わない。

### ✅ 2026-08-05 Codex: ウルク第4D・実測水利紛争とH4/H7販促実験

**ゲーム開発**

- 水利要求を固定第8期イベントから、請求側農地の不足または水路劣化と、同期間のウルク実流量が同時観測された場合だけ発生するイベントへ変更。発生時の対象農地、双方水路、必要／受水量、水路状態、ウルク流量、確度を保存する。
- 解決案を **分水・穀物補償・共同補修・拒否** の4つに分けた。補償は大麦2を双方の備蓄間で移し、共同補修は葦1と水利労働10%以上を要求して請求側水路を改善する。
- 分水は3期間、ウルク農地の実受水から不足分の範囲で毎期最大2を移す。請求側の自力補修で不足が解消すれば不要分は移さず、必要量と実施量を決済して完了／不履行を信頼・評判・外交履歴へ反映する。水量保存則は維持する。
- 地域UIに観測根拠と4解決ボタンを追加。個別事件の確定再現ではなく `inferred` な復元モデルと明示した。史実キャンペーン進捗はversion 6で、version 5の既存水利問題を農地・水路・解決種別付きへ移行する。

**販売促進**

- `MARKETING_EXPERIMENT_H4_H7.md` にX投稿12本を事前登録した。H4は「育っていく体験／史実4X」、H7は「ゲーム内容／AI共同開発」の2×2で各セル3本。30／80／150ターン画像を各セル1回ずつ使い、計12本を均衡化した。
- 全投稿へ一意の `utm_content` を割り当て、CTRだけでなく商品閲覧→購入完了と訪問者あたり手取りを判定する。P1本番行確認とP2画像公開までは投稿せず、価格・見出し・購入ボタンを同時変更しない。
- Obsidianの10仮説台帳に「未投稿・未検証」と開始ゲートを記録した。文案作成を検証結果と誤認せず、分母・分子・期間がそろうまでH4/H7を保留する。

**検証（Unity 6000.3.20f1）**

- `UrukRegionalSimulationSmokeTest.Run`: `URUK REGIONAL SIMULATION SMOKE OK`。物理条件による発生、4解決案、実備蓄移動、共同補修、3期間分水、不要水量の停止、完了／不履行、評判、v5→v6移行、決定的セーブ、水・物資保存を確認。3 seed性能は0.10 ms/turn。
- `UrukCampaignVerticalSliceSmokeTest.Run`: `URUK CAMPAIGN VERTICAL SLICE SMOKE OK`。
- `SmokeTest.Run`: `SMOKE OK`。変更前 `Logs/g4_regression.log` と `[Smoke]` / `SMOKE` 全19行がdiff完全一致。
- 製品版 `BUILD OK: 98,055,413 bytes`、体験版 `DEMO BUILD OK: 98,055,413 bytes`。両方を12秒起動し、Unity 6000.3.20f1・重大例外0。
- 製品ZIP: 37,583,034 bytes、SHA256 `ccf047d6fb92cadfc7add6bbae95b67e0ba8da2c20f0d589ae2c77f467ea4e5d`。
- 体験版ZIP: 37,585,735 bytes、SHA256 `aa8ae86d274add463a529275be0c4a552724f032a42dbb99fb3eca19677db090`。

**次の担当**

- Claude: 承認後に商品画像PR #4408の生成リリースノートを修正・CI再実行・マージし、P1 SQL確認とP2公開日をObsidianへ返す。H4/H7は開始ゲート通過後に投稿順と時刻だけ確定する。
- Codex: 複数の水利要求、共同水路の管理権、破約後の再交渉へ進む。外部投稿や価格変更は実験開始記録なしに行わない。
...
- **制限方式はターン数**(ユーザー決定)。既定 **30 ターン**で終了し、続きは製品版へ、という案内を出す。
- 30 ターンという数字は、**「育っていくのを見る」体験の入口までは届き、続きが気になるところで切れる**ことを狙っている(このゲームの価値は [[project_hexciv_value_is_watching_it_grow]] のとおり「増えたものが即座に分かること」にある)。実際に遊んでみて早すぎ/遅すぎと感じたら**数字は調整してよい**。判断はそちらに委ねる。
- 起動時に体験版であることが分かるようにする。終了時は「ここまでが体験版です」と、**何が製品版で続くのか**を具体的に示す(残りターン数、未到達の時代、未解放の要素など)。
- ビルドの出し分け方法は任せる。ただし **`scripts/pack_release.ps1` が製品版 zip を作る前提**なので、体験版を別 zip として出せる形にしてほしい(引数追加でもよい)。

**守ってほしい不変条件**(これまでと同じ)

- シミュレーションの結果を変えない。`SmokeTest.Run` の `[Smoke]`/`SMOKE` 全19行が基準ログと **diff 完全一致**すること(体験版はターン上限で止めるだけで、途中までの挙動は製品版と同一であるべき)。
- ヘッドレスで例外ゼロ。
- 体験版でも**セーブデータの互換を壊さない**(体験版で遊んだ人が製品版を買って続けられるのが理想。難しければその旨を書いてほしい)。

**Claude Code 側は別領域**: サイトの販売計測(funnel)・商品ページ・スクリーンショット自動生成・itch.io 掲載素材を担当する。`Assets/` は触らないので競合しない。

### 📦 2026-07-28 Claude Code 第23弾: 有償配布の準備(LICENSE + リリースパッケージ)

**方針転換**: HexCiv を「自分株式会社」のサイト(<https://my-web-app-b67f4.web.app/>)から**有料でダウンロード販売**する。ユーザー決定により、**リポジトリは公開のまま**・価格 **¥500**・配信は Supabase の非公開バケット + 署名付きURL・購入はサイトへログインしてから、とする。

- **`LICENSE.md`(新規)**: 公開リポジトリのまま売るため、意図を明文化した source-available ライセンス。閲覧・フォーク・**個人利用のビルド**は許可し、**ビルド済みバイナリの再配布と商用利用は禁止**する。LICENSE が無い状態は法的には「全権利保有」だが意図が伝わらないため明示した。貢献者の PR も同条件で有償配布に含まれることを明記。Unity 本体の条項は変更しない旨も併記。
- **`scripts/pack_release.ps1`(新規)**: `Build/` を `HexCiv-v<版>-win64.zip` に固め、SHA256・バイト数・生成時刻を manifest(JSON)に出す。配信経路に依存しない前提条件。版番号は未指定なら `ProjectSettings` の `bundleVersion` を読む。**空や欠損のビルドを配ってしまわないよう**、実行ファイル・データフォルダ・`Assembly-CSharp.dll` の実在を先に検証して落とす。`UnityCrashHandler64.exe` と `.pdb` は同梱しない。出力先 `dist/` は .gitignore 済み。

**実測(2026-07-28)**: `PACK OK` — 144ファイル(除外1)、**37,572,177 bytes(35.8MB)**、展開後 91.9MB。SHA256 `cc0e5caa…`。zip を実際に展開して必須ファイル5点の実在・`.pdb` 0件・クラッシュハンドラ非同梱を確認し、**展開したビルドを起動して例外ゼロ**(Unity 6000.3.20f1)まで確認した。

**注意(Codex 向け)**: `.ps1` を UTF-8 で保存するときは **BOM 必須**。Windows PowerShell 5.1 は BOM 無しを ANSI(日本語環境では Shift-JIS)として読むため、日本語コメントが壊れて構文エラーになる(今回実際に踏んだ)。
...
- **記録が空のままになる**: 基準値だけあって履歴が空、という状態は普通に起こる(この仕組みが入る前から遊んでいた場合など)。このままでは次に何かが増えるまで「成長の記録」がずっと空で、入口として機能しない。`GrowthHistory.EnsureOrigin()` を追加し、記録が1件も無いときだけ現在の規模を「起点」として置く。呼ぶのは**カードの構築より後**で、差分があった起動では直前の確定で更新が記録済みのため起点は作られない(増えた後の値を「起点」と称して二重に置かない)。閲覧画面を開いたときにも同じ保険をかけている。
- **何が増えたのか言えなくなる**: 記録していたのは件数だけだったため、後から言えるのは「新しいパネル 1」まで。**どれが増えたのかは次の起動で基準値が進んだ時点で永久に失われる**。`GrowthRecord` に `NewPanelNames` / `NewCoreTypeNames` を追加し、一覧に「新しいパネル: 領土の変遷」と名前で出す。名前を持たない旧記録は件数表示へ自動的に落ちるので、既存の記録が読めなくなることはない。

**検証(Unity 6000.3.20f1 / 2026-07-28)**: コンパイル error 0 / warning 0。`GROWTH HISTORY SMOKE OK`(起点は空のときだけ1件・増分0・null 安全、名前の保存と**読み戻しの往復**、閲覧画面を空で開くと起点が置かれること)。`SmokeTest` 全19行が基準ログと **diff 完全一致**。`BUILD OK: 98030837 bytes`。**実起動**で例外ゼロ、差分ゼロの起動で起点が1件だけ記録され(`IsOrigin:true` / 増分すべて0 / `CoreTypes:24`)、第20弾の Core 修正が実機の記録にも反映されていることを確認した。

### ✅ 2026-07-27 Claude Code 第20弾: 「成長の記録」+ Core 指標が常に0だった欠陥の修正

**背景**: 第19弾の「今回の更新」カードは**前回起動からの差分だけ**を出すため、一度見ると消えて二度と戻らない。増えたものが**積み上がっていく履歴として残る**ようにした。

**実装**

- 新規 `UI/GrowthHistory.cs`: 差分の確定に成功するたび1件記録(日時・各総数・増分・要約)。PlayerPrefs に JSON で保存し、120件を超えたら古い順に捨てる。初回は「起点」として総数のみ記録(グラフの始点になる)。**差分ゼロは記録しない**。
- 新規 `UI/GrowthHistoryView.cs`: タイトル左下の「成長の記録」から開く閲覧画面。「世界の大きさ」(台帳+規則+パネル+仕組み)の推移グラフ + 更新履歴の一覧(ページ送り)。縦軸は0始まりではないので、**上端・下端の実数値をグラフ内に明示**して誤読を防ぐ。
- `UI/TitleScreen.cs`: 常設ボタン(差分ゼロの日でも開ける)。開いている間は Esc をタイトルに渡さない。ゲームへ遷移するときは記録画面を畳む。
- `UI/UpdateDigest.cs`: `Commit()` 成功時に履歴へ1件記録(基準値の前進と1対1)。

**実測で見つけた欠陥の修正(第19弾の不具合)**: `core.systems` / `core.catalogs` が**実機で常に 0**だった。原因は型走査の `type.IsAbstract` による除外で、C# の `static class` は IL 上 **abstract sealed** になるため、Core の System / Catalog **24件すべて**が落ちていた。エラーも警告も出ず「Coreの仕組み」行が出ないだけなので、実機のレジストリを見るまで気づけない種類の欠陥だった。`IsAbstract && !IsSealed`(=継承前提の抽象基底だけ)へ修正し、**12 + 12 = 24件**を拾えることを確認。

- **移行ガード**: 修正だけ入れると既存ユーザーに「新しい仕組み +24」という**実際には何も増えていない偽の更新**が出る。`core.scan.generation` を導入し、世代が変わった直後の起動では Core 系を**静かに基準値化するだけ**にして報告しない。
- **回帰ガード**: 新規 `Editor/UpdateDigestSmokeTest.cs` が各指標を「正の値であること」で固定する。同じ壊れ方が二度と静かに通らない。

**検証(Unity 6000.3.20f1 / 2026-07-27)**

- バッチコンパイル: **error 0 / warning 0**、終端 `Exiting batchmode successfully now!` を確認。`Assembly-CSharp.dll` が実際に再生成されたこと(時刻・サイズ変化)も確認した。
- 新規 `GrowthHistorySmokeTest.Run`: `GROWTH HISTORY SMOKE OK`。起点/更新の時系列保持、差分ゼロを記録しない、上限120件で古い順に捨てる、**壊れた JSON を掴んでも例外を出さず記録を積み直せる**、閲覧画面が記録あり/なしの両方で例外なく組み上がる、を検証。
- 新規 `UpdateDigestSmokeTest.Run`: `UPDATE DIGEST SMOKE OK`(`core.systems=12 core.catalogs=12 ui.panels=15 ledger.total=1303`)。
- `SmokeTest.Run`: `SMOKE OK`。`[Smoke]`/`SMOKE` 全19行が基準 `Logs/stage4a_normal150.log` と **diff 完全一致** = シミュレーション不変。
- ビルド: `BUILD OK: 98029301 bytes`。
- **実起動テスト**: 例外ゼロ。台帳を巻き戻して起動 → 履歴が JSON で正しく記録され(`"台帳 +8"`)、**同じ起動で Core は 0→12/12・フラグ24件・世代2 へ静かに移行し、偽の「新しい仕組み +24」は出ていない**(`NewCoreTypes:0` / `TotalDelta:8`)。移行ガードが実機で機能することを確認した。
- Core/ は**一切変更していない**。検証で作った架空の履歴レコードは実機から削除済み。

**Codexへ**: **これまで通り自由に開発を続けてください**。触ったのは `UI/` の4ファイル(新規2 + 既存2)と `Editor/` の新規テスト2件だけです。
...
- 食料産出は `基礎収量 × 労働 × 灌漑 × 土壌 × 種籾・道具 × 氾濫`。運河60%以上で農地2区画を完全灌漑、30～59%で1区画、30%未満で停止。次期産出予測と各倍率をUIへ表示する。
- 公共労働は世帯維持を除く動員可能人口を、食料・運河・建設・工芸・交易・民兵へ5%刻み、合計100%で配分。通常操作では食料30%を保護し、自動管理は食料安全だけを扱って強制・債務・捕虜・奴隷化・権利変更を行わない。
- 人口を出生－通常死亡－危機死亡＋流入－流出で更新。長期的役割と当期労働を分離。自由・依存・債務拘束・捕虜・奴隷化状態を別フィールドにし、今回の自動地位変化は食料支援への依存化と危機解消後の復帰だけ。人数・原因・確度を追記イベントへ保存し、丸め誤差で奴隷化を生成しない。
- 社会状態を食料、労働、配給、格差、信頼、祭祀、外部安全、水利、直近衝撃の9要因へ分解。安定20未満または人口5%以上の流出を重大不安とし、3期連続で分裂、安定35以上かつ危機解消で回復する。
- 初期神殿区画を敷地、日干し煉瓦・葦、基壇・壁、祭祀空間・倉庫、配給・記録担当者の5段階へ変更。各段階で物資と建設労働を使い、中断時は未完成部分が劣化する。「ジッグラト」とは呼ばない。
- 神殿完成後に「数量記録行政」を採用可能。原文字・文字完成とは区別し、行政前は人口・備蓄を推定幅、行政後は狭い推定値で表示。都市国家成立は人口2,500、完全灌漑2、食料安全2期、神殿5段階、数量記録行政を要求し、推奨経路でターン8～10に成立する。
- 進捗をversion 2へ更新し、第2段階version 1セーブを人口・備蓄・施設進捗を保ったまま移行。専用UIに労働配分、食料／人口／地位／社会内訳、最大リスク・原因・最大3行動・予測を追加。
- 検証: `URUK CAMPAIGN VERTICAL SLICE SMOKE OK`（食料台帳、5%労働、人口内訳、地位イベント、重大不安、神殿5段階、都市国家化、version 1移行、決定的セーブを含む）、`HISTORICAL CAMPAIGN FOUNDATION SMOKE OK`、通常150ターン `SMOKE OK`（turn150 units=88 cities=18 techs=165 wars=3）、Windows `BUILD OK: 97955329 bytes, 11.7s`、Unity 6000.3.20f1で18秒ヘッドレス起動し重大例外0。
- Claude Code第19弾の宣言対象 `UI/TitleScreen.cs` / `UI/UpdateDigest.cs` / `Audio/GameAudio.cs` は編集・ステージしていない。検証と正式Buildには共有ワークツリー時点の変更を含む。次は完全タイル操作、同一詳細ルールを使う7勢力AI、実物資型の贈与・交換・貸付・朝貢。
