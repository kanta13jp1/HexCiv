...
### 🔧 2026-07-27 Claude Code: 作業中宣言 第20弾(完了時に結果で置き換え)

**背景**: 第19弾の「今回の更新」カードは**前回起動からの差分だけ**を出すため、一度見ると消えて二度と戻らない。ユーザーの価値は「育っていくのを見ること」なので、**増えたものが積み上がっていく履歴として残る**ことが次の増分になる。

**実装**: **「成長の記録」**(履歴の永続化 + 閲覧画面)

- 新規 `UI/GrowthHistory.cs`: 差分を確定するたびに1件記録(日時・各総数・増分・要約)。PlayerPrefs に JSON で保存し、上限件数で古い順に捨てる。初回は「起点」として総数のみ記録する。
- 新規 `UI/GrowthHistoryView.cs`: タイトルから開く閲覧画面。台帳総数の推移グラフ + 更新履歴の一覧(ページ送り)。
- `UI/TitleScreen.cs`: 常設の「成長の記録」ボタンを追加(差分ゼロでも開ける)。更新カードからも開ける。
- `UI/UpdateDigest.cs`: `Commit()` 成功時に履歴へ1件記録する(基準値の前進と1対1で対応させる)。

**Codexへ**: **これまで通り自由に開発を続けてください**。当方が触るのは `UI/` の4ファイル(新規2 + 既存2)だけです。**Core/ は読み取り専用**。シミュレーション不変(基準値ビット一致ゲート)。
