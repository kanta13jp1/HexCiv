...
| Historical campaign | `Core/HistoricalCampaign.cs, Core/HistoricalContentSchema.cs, Core/HistoricalCampaignFactory.cs, Core/HistoricalCampaignSave.cs, Core/UrukCampaignSystem.cs` — 固定史実マップ、可変年代、8勢力、小集落1,500人、役割・地位・物資・施設、ウルク都市国家化、データセット版数、出典・確度・権利・生成素材の製品収録ゲート。Core純度と決定性は通常Simulationと同じ |
...
| Historical campaign UI | `UI/HistoricalCampaignPanel.cs` — 年代・実人口・食料・農地・運河・神殿・安定度、導入顧問、簡潔／詳細、史実クイックセーブ、暫定ローポリ復元模型。sortingOrder 122の独立Canvas |
