...
| Simulation | `Core/GameStateOps.cs, Core/Player.cs, Core/Unit.cs, Core/City.cs, Core/Combat.cs, Core/Pathfinder.cs, Core/Visibility.cs, Core/TurnManager.cs, Core/SaveLoad.cs, Core/CivilizationCatalog.cs, Core/LeaderCatalog.cs, Core/HeritageSiteCatalog.cs, Core/GreatPersonCatalog.cs, Core/ResearchMilestoneCatalog.cs, Core/CulturalTraditionCatalog.cs, Core/MasterpieceCatalog.cs, Core/GlobalHistoryIndex.cs, Core/TechnologyCatalog.cs, Core/CulturePolicyCatalog.cs, Core/CultureSystem.cs, Core/WorldLegacySystem.cs, Core/MasterpieceSystem.cs` (SaveLoad: 決定的なセーブ/ロード直列化。CivilizationCatalog: 80文明。LeaderCatalog: 指導者155件。HeritageSiteCatalog: 遺跡・史跡108件。GreatPersonCatalog: 偉人120人。ResearchMilestoneCatalog: 研究史120件。CulturalTraditionCatalog: 文化史120件。MasterpieceCatalog: 書籍・絵画・彫刻・建築・音楽・演劇・映画252件。GlobalHistoryIndex: 13分類・台帳955件の横断集計と文明詳細地域の6地域写像。TechnologyCatalog: 既存12技術+研究史120件。CulturePolicyCatalog/CultureSystem: 文化史120件の政策化、文化ポイント、交流、文化勝利。WorldLegacySystem: 遺産12件の決定的配置・発見報酬、偉人ポイント・世界一意の登用・分野別効果・文明地域親和性。MasterpieceSystem: 作品ポイント、世界一意の収蔵、7分野効果、偉人連携。各CatalogはMonoBehaviour非依存の純データ台帳) |
...
| UI | `UI/UIManager.cs, UI/UIStyle.cs, UI/WorldHistoryPanel.cs, UI/CulturePanel.cs, UI/LegacyPanel.cs` (WorldHistoryPanel: 総合・文明・指導者を含む世界史台帳8画面とゲーム内状態を閲覧し、`Resources/History/world_history_banner` と文明／指導者・遺跡／偉人・研究／文化・演劇／映画の2分割アイコンを任意表示する。CulturePanel: 文化政策・影響力。LegacyPanel: 遺産探索・偉人登用・作品収蔵。いずれもsortingOrder 130以上の独立Canvas。helpers allowed in `UI/`) |
...
Note: SaveLoad version 9 で遺産・偉人に加えて作品ポイント・累計・収蔵済み作品IDを永続化。version 8以前は作品進行を0／空で補完し、version 7以前は `TurnManager` 構築時にseed由来で遺産を決定的に補完する。
