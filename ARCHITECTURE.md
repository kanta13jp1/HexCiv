...
Unity 6.3 LTS (6000.3.20f1) / C# / no external assets. A Civilization-like 4X strategy prototype:
...
| Simulation | `Core/GameStateOps.cs, Core/Player.cs, Core/Unit.cs, Core/City.cs, Core/Combat.cs, Core/Pathfinder.cs, Core/Visibility.cs, Core/TurnManager.cs, Core/SaveLoad.cs, Core/CivilizationCatalog.cs, Core/LeaderCatalog.cs, Core/HeritageSiteCatalog.cs, Core/GreatPersonCatalog.cs, Core/ResearchMilestoneCatalog.cs, Core/CulturalTraditionCatalog.cs, Core/MasterpieceCatalog.cs, Core/MaterialCultureCatalog.cs, Core/HistoricVesselCatalog.cs, Core/NaturalFeatureCatalog.cs, Core/NaturalGeographySystem.cs, Core/GlobalHistoryIndex.cs, Core/TechnologyCatalog.cs, Core/CulturePolicyCatalog.cs, Core/CultureSystem.cs, Core/WorldLegacySystem.cs, Core/MasterpieceSystem.cs, Core/AdministrationSystem.cs, Core/LogisticsSystem.cs, Core/PopulationSystem.cs, Core/PoliticalSystem.cs, Core/MarketSystem.cs` (SaveLoad: 決定的なセーブ/ロード直列化。CivilizationCatalog: 92文明。LeaderCatalog: 指導者179件。HeritageSiteCatalog: 遺跡・史跡120件。GreatPersonCatalog: 偉人132人。ResearchMilestoneCatalog: 研究史132件。CulturalTraditionCatalog: 文化史132件。MasterpieceCatalog: 書籍・絵画・彫刻・建築・音楽・演劇・映画336件。MaterialCultureCatalog: 名産・料理・交通・武器・身体文化12分類72件。HistoricVesselCatalog: 歴史船舶36件。NaturalFeatureCatalog: 山・川・海・湖・森林・砂漠72件。NaturalGeographySystem: 内陸湖、流向河川、季節氾濫原、渡河、橋梁網、自然多様性、産出・科学・文化・市場効果。GlobalHistoryIndex: 32分類・台帳1303件の横断集計と文明詳細地域の6地域写像。TechnologyCatalog: 基礎14技術+研究史132件。CulturePolicyCatalog/CultureSystem: 文化史132件の政策化、文化ポイント、交流、文化勝利。WorldLegacySystem: 遺産12件の決定的配置・発見報酬、偉人ポイント・世界一意の登用・分野別効果・文明地域親和性。MasterpieceSystem: 作品ポイント、世界一意の収蔵、7分野効果、偉人連携。AdministrationSystem: 国庫・税制・維持費・安定度・戦争疲弊・総合産出倍率・AI税制勧告。LogisticsSystem: 都市起点の決定的補給網、港からの海上補給、艦船封鎖・局地護衛、護送船団、地形・領有・敵遮断、技術・穀物庫、逼迫・孤立・消耗・戦闘補正。PopulationSystem: 農民・工人・学者、需要、教育、満足度、社会重点、都市間移住。PoliticalSystem: 政治力・正統性・4利害集団・4法律・AI制定。MarketSystem: 5財、需要、在庫、価格、交易、4市場方針、生活技術72件の地域産業化。各Catalog/SystemはMonoBehaviour非依存の純データ/ロジック) |
| Historical campaign | `Core/HistoricalCampaign.cs, Core/HistoricalContentSchema.cs, Core/HistoricalCampaignFactory.cs, Core/HistoricalCampaignSave.cs, Core/UrukCampaignSystem.cs, Core/UrukSubsistenceSystem.cs, Core/UrukRegionalSystem.cs` — 固定史実マップ、可変年代、8勢力、小集落1,500人、役割・地位・物資・施設、5%刻み公共労働、備蓄月数・損失・種籾・容量、人口収支、原因付き地位変化、9要因社会安定、神殿5段階、数量記録行政、ウルク都市国家化、水利グラフ・農地・実物資輸送、贈与・物々交換・貸付・労務・通行権・朝貢の6契約、期限・不履行・輸送損失、食料危機AI貸付、合意・履行・不履行・水利判断の外交履歴、外交評判と低信頼時の契約拒否、実流量・農地不足・水路状態から発生する複数水利要求、未決・履行中・再交渉可能を横断する個別対象選択、推定水系の上流／下流関係、分水・穀物補償・4期間共同管理・拒否、複数期履行、選択対象だけへの破約、AI報復、輸送危険、選択相手だけへの現物再交渉、ニップール共同体による第三者仲裁、氏名不詳・双方同意前提の親族連携、口頭伝言・封泥付き荷・数量記録板による決定的な情報伝達、媒体別年代制限・現物消費・遅延・信頼度・誤解・交易危険軽減、伝達の交易5%・封緘／記録の工芸5%稼働枠、発送中の重複配置防止、氏名／人数不詳と推定表示、共同体・媒体別の受信処理力、媒体信頼度を重くした実効理解率、理解成否・交渉結果・信頼変化、到着済み情報と後続契約輸送の照合、媒体別の推定危険幅・数量条件精度・照合件数、データセット版数、出典・確度・権利・生成素材の製品収録ゲート。受信処理力と実効理解率は識字率・史実人数ではなく `inferred` なゲーム値。Core純度と決定性は通常Simulationと同じ |
...
| Historical campaign UI | `UI/HistoricalCampaignPanel.cs, UI/UrukRegionalPanel.cs` — 年代・推定人口幅・備蓄月数・農地・運河・神殿5段階・安定度、最大リスクと3行動の顧問、5%刻み労働配分、人口／食料／地位／社会内訳、簡潔／詳細、史実クイックセーブ、暫定ローポリ復元模型。地域パネルは水利・農地・物流レイヤー、6契約、履行期限、輸送状況、外交評判、最新の外交履歴、水利紛争の観測原因、未決・履行中・再交渉可能を横断する対象番号、上下流関係、仲裁者と分水／穀物補償／共同管理／拒否／破約／再交渉／第三者仲裁、土地・耕作権の観測根拠と共同耕作／現物補償／季節利用仲裁／拒否／破約／再交渉、親族連携候補・同意前提・確度・交易危険軽減、情報伝達先・媒体・媒体史料確度・個別シナリオ確度・到着期・信頼度・交易危険軽減、最新輸送の情報照合・数量条件・推定危険幅を表示。sortingOrder 122/123の独立Canvas |
...
`GameRules` (all data tables + formulas: `MoveCostInto`, `CanUnitEnter`, `DefenseBonusAt`, `GrowthFoodNeeded`, `CombatDamage`, `HealthScaledStrength`, constants),
...
Note: 史実キャンペーン進捗 version 14 で、情報伝達の相手処理力、実効理解率、受信成否、交渉結果、信頼変化を永続化する。version 13以前の解決済み伝達は旧記録として保持し、現在の処理力から理解成否を遡及再計算しない。version 13の担当役割・交易／工芸労働枠・担当者像、version 12の輸送情報照合、version 11の情報伝達、version 10の共同体間親族連携、version 9の農地権利・土地紛争、version 8の水利案件選択・推定水系・第三者仲裁、version 7の共同管理・破約・再交渉・報復、version 6の観測原因・複数期分水、version 5の外交履歴・評判は連鎖移行し、既存地域状態と契約債務を保持する。
...
public class Player {
    ...
    public SocialFocus SocialFocus;              // Balanced / Agriculture / Crafts / Learning
    public int PoliticalCapital; public int Legitimacy; public CivicLaw ActiveLaw;
    public int ScholarSupport; public int MerchantSupport; public int TraditionalSupport; public int MilitarySupport;
    public EconomicPolicy EconomicPolicy; public int FoodGoods, MaterialGoods, ManufacturedGoods, KnowledgeGoods, TransportGoods;
    public int MarketAccess, DemandFulfillment, LastImports, LastExports, LastTradeBalance, LastTradePartnerId;
    public string FeaturedIndustryId; public HashSet<string> DevelopedMaterialCultures;
...
public class City {
    ...
    public int Farmers; public int Artisans; public int Scholars; // sum always equals Population
    public int Education; public int Satisfaction;                // 0..100
    public int FoodNeedFulfillment; public int HousingNeedFulfillment; public int ServiceNeedFulfillment;
    public int LastNetMigration;                 // -1 / 0 / +1, current turn
...
```
`AdvanceTurn()` (private): TurnNumber++; `MarketSystem.AdvanceMarkets`（全文明の5財生産・需要消費・価格・交易・地域産業・AI市場方針）; for every player: units `ResetForNewTurn` (healing, goto continuation), cities `ProcessTurnStart` + `ExpandBordersIfNeeded`, `PopulationSystem.AdvancePlayer`（階層・需要・教育・満足・移住・AI重点）, `PoliticalSystem.AdvancePlayer`（政治力・正統性・支持・AI法律）, research progress, `CultureSystem.AdvancePlayer`, `WorldLegacySystem.AdvancePlayer`（遺産検査・偉人ポイント・AI登用）, `MasterpieceSystem.AdvancePlayer`（作品ポイント・AI収蔵）, `AdministrationSystem.AdvancePlayer`（国庫収支・交易収支・安定度・戦争疲弊）, visibility recompute, then elimination/制覇判定、`CultureSystem.AdvanceExchange` + `CheckCulturalVictory`、ターン上限スコア判定、`Bump()`。
...
```
- Continents/pangaea/archipelago: layered value noise (implement your own — `Mathf.PerlinNoise` is fine seeded via random offsets) with edge falloff so map borders are ocean. Target land fraction depends on map type.
...
- Local elevation depressions create 1–3 inland lakes. Mountains (hills only when a special map has no mountains) route 1–4 deterministic rivers to nearest water without consuming RNG. River land yields +1 Food; NaturalGeographySystem derives science/culture diversity and river/coastal/lakeside market access.
...
`ProductEdition` (root namespace `HexCiv`) owns the build-edition policy. `HEXCIV_DEMO` is supplied only by the demo build and enables a 30-completed-turn presentation gate. The gate must remain outside `Core`, must not set `GameState.IsGameOver`, and must not add fields to `GameConfig`/SaveLoad DTOs. This preserves identical simulation and lets a demo save continue in the full build. `UIManager.SetDemoMode` shows the persistent badge; `ShowDemoLimit` blocks further input and offers slot-1 saving.
...
- `BuildScript.PerformDemoBuild` (static): same scenes/target, output `BuildDemo/HexCiv-Demo.exe`, and pass `extraScriptingDefines={"HEXCIV_DEMO"}` for that build only.
- `DemoModeSmokeTest.Run` (static): compare full-reference/demo-candidate serialized checkpoints after 30 identical turns, assert the demo gate does not set `IsGameOver`, round-trip the save, and continue one more turn.
